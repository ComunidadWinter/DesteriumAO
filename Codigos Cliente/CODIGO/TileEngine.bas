...
Public Sub CargarAuras()
    Dim AuraPath As String, loopC As Long
    AuraPath = App.path & "\INIT\Auras.ini"
    MaxAuras = Val(GetVar(AuraPath, "INIT", "MaxAuras"))
    If MaxAuras > 0 Then
        ReDim Auras(0 To MaxAuras) As tAuras
 
        For loopC = 0 To MaxAuras
            With Auras(loopC)
...
'Cambiamos el viejo sistema de lluvias por uno mas facil de manipular (DIRIX)
Sub CargarArrayLluvia()
Dim n As Integer
Dim i As Long
n = 210 'Aqui si o si tienes que colocar la cantidad de mapas que cargara el cliente


'Resize array
ReDim bLluvia(1 To n) As Byte

For i = 1 To n
bLluvia(i) = GetVar(App.path & "\init\" & "Fk.DS", "LLUVIA", "L" & i)
Next i
...
'TODO : Check this!!
    If bLluvia(UserMap) = 1 Then
        If bRain Then
            ...
            For Y = 0 To 4
                For X = 0 To 4
                If (CurMap = 170) Or (CurMap = 171) Or (CurMap = 169) Then
                    Call BackBufferSurface.BltFast(LTLluvia(Y), LTLluvia(X), SurfaceDB.Surface(5557), RLluvia(iFrameIndex), DDBLTFAST_SRCCOLORKEY + DDBLTFAST_WAIT)
                    Else
...
'***************************************************
    ...
    With charlist(CharIndex)
        ...
        For loopxx = 0 To 4
            ...
            If .aura(loopxx).AuraGrh.GrhIndex > 0 Then
            
            .aura(loopxx).AuraGrh.FrameCounter = 1
            
                '#If ConAlfaB = 1 Then
                 '   Call DDrawTransGrhtoSurfaceAlpha(.aura(loopxx).AuraGrh, PixelOffsetX + .aura(loopxx).OffSetX, PixelOffsetY + .aura(loopxx).OffSetY, 1, 1)

                '#Else
                    Call DDrawTransGrhtoSurface(.aura(loopxx).AuraGrh, PixelOffsetX + .aura(loopxx).OffSetX, PixelOffsetY + .aura(loopxx).OffSetY, 1, 1)
                '#End If
            End If
        Next loopxx
