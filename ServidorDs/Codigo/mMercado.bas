...
' @ Compramos el personaje seleccionado
    ...
    With UserList(Userindex)
        ...
        tempUser = NameIndex(Mercado(IndexMercado).Recibidor)
        If tempUser > 0 Then
            UserList(tempUser).Stats.GLD = UserList(tempUser).Stats.GLD + Mercado(IndexMercado).valor
            WriteUpdateGold tempUser
        Else
...
' @ Publicamos el personaje elegido
    With UserList(Userindex)
        ...
        If .flags.IndexMercado > 0 Then Exit Sub    'Ta?
