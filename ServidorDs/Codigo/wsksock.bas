...
10        If (lParam And &HFFFF&) > &H7FFF Then
20            WSAGetAsyncBufLen = (lParam And &HFFFF&) - &H10000
30        Else
40            WSAGetAsyncBufLen = lParam And &HFFFF&
50        End If
End Function

Public Function WSAGetSelectEvent(ByVal lParam As Long) As Integer
10        If (lParam And &HFFFF&) > &H7FFF Then
20            WSAGetSelectEvent = (lParam And &HFFFF&) - &H10000
30        Else
40            WSAGetSelectEvent = lParam And &HFFFF&
50        End If
End Function
...
10        WSAGetAsyncError = (lParam And &HFFFF0000) \ &H10000
End Function
...
10    tmp = GetAscIP(GetHostByNameAlias(AddrOrIP$))
20    t = Split(tmp, ".")
30    AddrToIP = t(3) & "." & t(2) & "." & t(1) & "." & t(0)
...
10        SockReadBuffer$ = vbNullString
20        sockin = saZero
30        sockin.sin_family = AF_INET
40        sockin.sin_port = htons(Port)
50        If sockin.sin_port = INVALID_SOCKET Then
60            ConnectSock = INVALID_SOCKET
70            Exit Function
80        End If

90        sockin.sin_addr = GetHostByNameAlias(Host$)
100       If sockin.sin_addr = INADDR_NONE Then
110           ConnectSock = INVALID_SOCKET
120           Exit Function
130       End If
140       retIpPort$ = GetAscIP$(sockin.sin_addr) & ":" & ntohs(sockin.sin_port)

150       S = Socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)
160       If S < 0 Then
170           ConnectSock = INVALID_SOCKET
180           Exit Function
190       End If
200       If SetSockLinger(S, 1, 0) = SOCKET_ERROR Then
210           If S > 0 Then
220               dummy = apiclosesocket(S)
230           End If
240           ConnectSock = INVALID_SOCKET
250           Exit Function
260       End If
270       If Not Async Then
280           If Not connect(S, sockin, sockaddr_size) = 0 Then
290               If S > 0 Then
300                   dummy = apiclosesocket(S)
310               End If
320               ConnectSock = INVALID_SOCKET
330               Exit Function
340           End If
350           If HWndToMsg <> 0 Then
360               SelectOps = FD_READ Or FD_WRITE Or FD_CONNECT Or FD_CLOSE
370               If WSAAsyncSelect(S, HWndToMsg, ByVal 1025, ByVal SelectOps) Then
380                   If S > 0 Then
390                       dummy = apiclosesocket(S)
400                   End If
410                   ConnectSock = INVALID_SOCKET
420                   Exit Function
430               End If
440           End If
450       Else
460           SelectOps = FD_READ Or FD_WRITE Or FD_CONNECT Or FD_CLOSE
470           If WSAAsyncSelect(S, HWndToMsg, ByVal 1025, ByVal SelectOps) Then
480               If S > 0 Then
490                   dummy = apiclosesocket(S)
500               End If
510               ConnectSock = INVALID_SOCKET
520               Exit Function
530           End If
540           If connect(S, sockin, sockaddr_size) <> -1 Then
550               If S > 0 Then
560                   dummy = apiclosesocket(S)
570               End If
580               ConnectSock = INVALID_SOCKET
590               Exit Function
600           End If
610       End If
620       ConnectSock = S
...
10        Linger.l_onoff = OnOff
20        Linger.l_linger = LingerTime
30        If setsockopt(SockNum, SOL_SOCKET, SO_LINGER, Linger, 4) Then
40            Debug.Print "Error setting linger info: " & WSAGetLastError()
50            SetSockLinger = SOCKET_ERROR
60        Else
70            If getsockopt(SockNum, SOL_SOCKET, SO_LINGER, Linger, 4) Then
80                Debug.Print "Error getting linger info: " & WSAGetLastError()
90                SetSockLinger = SOCKET_ERROR
100           Else
110               Debug.Print "Linger is on if nonzero: "; Linger.l_onoff
...
Sub EndWinsock()
          Dim ret&
10        If WSAIsBlocking() Then
20            ret = WSACancelBlockingCall()
30        End If
40        ret = WSACleanup()
50        WSAStartedUp = False
End Sub
...
Public Function GetAscIP(ByVal inn As Long) As String
    ...
    #End If
          Dim lpStr&
          Dim retString$
10        retString = String(32, 0)
20        lpStr = inet_ntoa(inn)
30        If lpStr Then
40            nStr = lstrlen(lpStr)
50            If nStr > 32 Then nStr = 32
60            MemCopy ByVal retString, ByVal lpStr, nStr
70            retString = Left$(retString, nStr)
80            GetAscIP = retString
90        Else
100           GetAscIP = "255.255.255.255"
110       End If
End Function
...
Public Function GetHostByAddress(ByVal addr As Long) As String
          ...
          Dim HostName$
10        phe = gethostbyaddr(addr, 4, PF_INET)
20        If phe Then
30            MemCopy heDestHost, ByVal phe, hostent_size
40            HostName = String(256, 0)
50            MemCopy ByVal HostName, ByVal heDestHost.h_name, 256
60            GetHostByAddress = Left$(HostName, InStr(HostName, Chr$(0)) - 1)
70        Else
80            GetHostByAddress = WSA_NoName
90        End If
End Function
...
Public Function GetHostByNameAlias(ByVal HostName$) As Long
          ...
          Dim addrList&
          Dim retIP&
10        retIP = inet_addr(HostName$)
20        If retIP = INADDR_NONE Then
30            phe = gethostbyname(HostName$)
40            If phe <> 0 Then
50                MemCopy heDestHost, ByVal phe, hostent_size
60                MemCopy addrList, ByVal heDestHost.h_addr_list, 4
70                MemCopy retIP, ByVal addrList, heDestHost.h_length
80            Else
90                retIP = INADDR_NONE
100           End If
110       End If
120       GetHostByNameAlias = retIP
End Function

'returns your local machines name
Public Function GetLocalHostName() As String
          Dim sName$
10        sName = String(256, 0)
20        If gethostname(sName, 256) Then
30            sName = WSA_NoName
40        Else
50            If InStr(sName, Chr$(0)) Then
60                sName = Left$(sName, InStr(sName, Chr$(0)) - 1)
70            End If
80        End If
90        GetLocalHostName = sName
End Function
...
#End If
          Dim sa As sockaddr
10        AddrLen = sockaddr_size
20        If getpeername(S, sa, AddrLen) Then
30            GetPeerAddress = vbNullString
40        Else
50            GetPeerAddress = SockAddressToString(sa)
60        End If
End Function
...
10        If val(PortStr$) > 32767 Then
20            GetPortFromString = CInt(val(PortStr$) - &H10000)
30        Else
40            GetPortFromString = val(PortStr$)
50        End If
60        If Err Then GetPortFromString = 0
End Function
...
10        ppe = getprotobyname(Protocol)
20        If ppe Then
30            MemCopy peDestProt, ByVal ppe, protoent_size
40            GetProtocolByName = peDestProt.p_proto
50        Else
60            tmpShort = val(Protocol)
70            If tmpShort Then
80                GetProtocolByName = htons(tmpShort)
90            Else
100               GetProtocolByName = SOCKET_ERROR
110           End If
120       End If
End Function
...
10        pse = getservbyname(service, Protocol)
20        If pse Then
30            MemCopy seDestServ, ByVal pse, servent_size
40            GetServiceByName = seDestServ.s_port
50        Else
60            Serv = val(service)
70            If Serv Then
80                GetServiceByName = htons(Serv)
90            Else
100               GetServiceByName = INVALID_SOCKET
110           End If
120       End If
End Function
...
#End If
          Dim sa As sockaddr
          Dim szRet$
10        szRet = String(32, 0)
20        AddrLen = sockaddr_size
30        If getsockname(S, sa, AddrLen) Then
40            GetSockAddress = vbNullString
50        Else
60            GetSockAddress = SockAddressToString(sa)
70        End If
End Function
...
20        Select Case errnum
              Case 10004: GetWSAErrorString = "Interrupted system call."
30            Case 10009: GetWSAErrorString = "Bad file number."
40            Case 10013: GetWSAErrorString = "Permission Denied."
50            Case 10014: GetWSAErrorString = "Bad Address."
60            Case 10022: GetWSAErrorString = "Invalid Argument."
70            Case 10024: GetWSAErrorString = "Too many open files."
80            Case 10035: GetWSAErrorString = "Operation would block."
90            Case 10036: GetWSAErrorString = "Operation now in progress."
100           Case 10037: GetWSAErrorString = "Operation already in progress."
110           Case 10038: GetWSAErrorString = "Socket operation on nonsocket."
120           Case 10039: GetWSAErrorString = "Destination address required."
130           Case 10040: GetWSAErrorString = "Message too long."
140           Case 10041: GetWSAErrorString = "Protocol wrong type for socket."
150           Case 10042: GetWSAErrorString = "Protocol not available."
160           Case 10043: GetWSAErrorString = "Protocol not supported."
170           Case 10044: GetWSAErrorString = "Socket type not supported."
180           Case 10045: GetWSAErrorString = "Operation not supported on socket."
190           Case 10046: GetWSAErrorString = "Protocol family not supported."
200           Case 10047: GetWSAErrorString = "Address family not supported by protocol family."
210           Case 10048: GetWSAErrorString = "Address already in use."
220           Case 10049: GetWSAErrorString = "Can't assign requested address."
230           Case 10050: GetWSAErrorString = "Network is down."
240           Case 10051: GetWSAErrorString = "Network is unreachable."
250           Case 10052: GetWSAErrorString = "Network dropped connection."
260           Case 10053: GetWSAErrorString = "Software caused connection abort."
270           Case 10054: GetWSAErrorString = "Connection reset by peer."
280           Case 10055: GetWSAErrorString = "No buffer space available."
290           Case 10056: GetWSAErrorString = "Socket is already connected."
300           Case 10057: GetWSAErrorString = "Socket is not connected."
310           Case 10058: GetWSAErrorString = "Can't send after socket shutdown."
320           Case 10059: GetWSAErrorString = "Too many references: can't splice."
330           Case 10060: GetWSAErrorString = "Connection timed out."
340           Case 10061: GetWSAErrorString = "Connection refused."
350           Case 10062: GetWSAErrorString = "Too many levels of symbolic links."
360           Case 10063: GetWSAErrorString = "File name too long."
370           Case 10064: GetWSAErrorString = "Host is down."
380           Case 10065: GetWSAErrorString = "No route to host."
390           Case 10066: GetWSAErrorString = "Directory not empty."
400           Case 10067: GetWSAErrorString = "Too many processes."
410           Case 10068: GetWSAErrorString = "Too many users."
420           Case 10069: GetWSAErrorString = "Disk quota exceeded."
430           Case 10070: GetWSAErrorString = "Stale NFS file handle."
440           Case 10071: GetWSAErrorString = "Too many levels of remote in path."
450           Case 10091: GetWSAErrorString = "Network subsystem is unusable."
460           Case 10092: GetWSAErrorString = "Winsock DLL cannot support this application."
470           Case 10093: GetWSAErrorString = "Winsock not initialized."
480           Case 10101: GetWSAErrorString = "Disconnect."
490           Case 11001: GetWSAErrorString = "Host not found."
500           Case 11002: GetWSAErrorString = "Nonauthoritative host not found."
510           Case 11003: GetWSAErrorString = "Nonrecoverable error."
520           Case 11004: GetWSAErrorString = "Valid name, no data record of requested type."
530           Case Else:
540       End Select
End Function
...
10        On Error Resume Next
20        IpToAddr = GetHostByAddress(GetHostByNameAlias(AddrOrIP$))
30        If Err Then IpToAddr = WSA_NoName
End Function
...
#End If
          Dim retString$
          Dim inn&
10        If val(IPL) > 2147483647 Then
20            inn = val(IPL) - 4294967296#
30        Else
40            inn = val(IPL)
50        End If
60        inn = ntohl(inn)
70        retString = String(32, 0)
80        lpStr = inet_ntoa(inn)
90        If lpStr = 0 Then
100           IrcGetAscIp = "0.0.0.0"
110           Exit Function
120       End If
130       nStr = lstrlen(lpStr)
140       If nStr > 32 Then nStr = 32
150       MemCopy ByVal retString, ByVal lpStr, nStr
160       retString = Left$(retString, nStr)
170       IrcGetAscIp = retString
180       Exit Function
IrcGetAscIPError:
190       IrcGetAscIp = "0.0.0.0"
200       Exit Function
210       Resume
End Function

Public Function GetLongIp(ByVal IPS As String) As Long
10        GetLongIp = inet_addr(IPS)
End Function
...
Function IrcGetLongIp(ByVal AscIp$) As String
    ...
    On Error GoTo IrcGetLongIpError:
          Dim inn&
10        inn = inet_addr(AscIp)
20        inn = htonl(inn)
30        If inn < 0 Then
40            IrcGetLongIp = CVar(inn + 4294967296#)
50            Exit Function
60        Else
70            IrcGetLongIp = CVar(inn)
80            Exit Function
90        End If
100       Exit Function
IrcGetLongIpError:
...
#End If
          Dim sockin As sockaddr
10        sockin = saZero     'zero out the structure
20        sockin.sin_family = AF_INET
30        sockin.sin_port = htons(Port)
40        If sockin.sin_port = INVALID_SOCKET Then
50            ListenForConnect = INVALID_SOCKET
60            Exit Function
70        End If
80        If LenB(Enlazar) = 0 Then
90            sockin.sin_addr = htonl(INADDR_ANY)
100       Else
110           sockin.sin_addr = inet_addr(Enlazar)
120       End If
130       If sockin.sin_addr = INADDR_NONE Then
140           ListenForConnect = INVALID_SOCKET
150           Exit Function
160       End If
170       S = Socket(PF_INET, SOCK_STREAM, 0)
180       If S < 0 Then
190           ListenForConnect = INVALID_SOCKET
200           Exit Function
210       End If
...
220       If bind(S, sockin, sockaddr_size) Then
230           If S > 0 Then
240               dummy = apiclosesocket(S)
250           End If
260           ListenForConnect = INVALID_SOCKET
270           Exit Function
280       End If
      '    SelectOps = FD_READ Or FD_WRITE Or FD_CLOSE Or FD_ACCEPT
290       SelectOps = FD_READ Or FD_CLOSE Or FD_ACCEPT
300       If WSAAsyncSelect(S, HWndToMsg, ByVal 1025, ByVal SelectOps) Then
310           If S > 0 Then
320               dummy = apiclosesocket(S)
330           End If
340           ListenForConnect = SOCKET_ERROR
350           Exit Function
360       End If
          
          'If listen(s, 5) Then
370       If listen(S, SOMAXCONN) Then
380           If S > 0 Then
390               dummy = apiclosesocket(S)
400           End If
410           ListenForConnect = INVALID_SOCKET
420           Exit Function
430       End If
440       ListenForConnect = S
End Function
...
10        TheMsg = vbNullString
20        Select Case VarType(vMessage)
              Case 8209   'byte array
30                sTemp = vMessage
40                TheMsg = sTemp
50            Case 8      'string, if we recieve a string, its assumed we are linemode
            #If Win32 Then
60                    sTemp = StrConv(vMessage, vbFromUnicode)
            #Else
70                    sTemp = vMessage
            #End If
80            Case Else
90                sTemp = CStr(vMessage)
            #If Win32 Then
100                   sTemp = StrConv(vMessage, vbFromUnicode)
            #Else
110                   sTemp = vMessage
            #End If
120       End Select
130       TheMsg = sTemp
140       If UBound(TheMsg) > -1 Then
150           kSendData = send(S, TheMsg(0), UBound(TheMsg) + 1, 0)
160       End If
End Function

Public Function SockAddressToString(sa As sockaddr) As String
10        SockAddressToString = GetAscIP(sa.sin_addr) & ":" & ntohs(sa.sin_port)
End Function

Public Function StartWinsock(sDescription As String) As Boolean
          Dim StartupData As WSADataType
10        If Not WSAStartedUp Then
              'If Not WSAStartup(&H101, StartupData) Then
20            If Not WSAStartup(&H202, StartupData) Then  'Use sockets v2.2 instead of 1.1 (Maraxus)
30                WSAStartedUp = True
...
40                sDescription = StartupData.szDescription
50            Else
60                WSAStartedUp = False
70            End If
80        End If
90        StartWinsock = WSAStartedUp
End Function

Public Function WSAMakeSelectReply(TheEvent%, TheError%) As Long
10        WSAMakeSelectReply = (TheError * &H10000) + (TheEvent And &HFFFF&)
End Function
