...
Option Explicit
Public ClaveActual As Byte
Public TieneClan As Boolean
Public SeguroClanes As Boolean
              
Private Type tAccount
    Name As String
    Passwd As String
    State As Byte
End Type

Public DataAccount As tAccount
...
End Function
Public Function DirBMP() As String
10        DirBMP = App.path & "\RECURSOS\GRAPHICS_BMP\"
End Function
Public Function DirDDS() As String
10        DirDDS = App.path & "\RECURSOS\GRAPHICS_DDS\"
End Function
...
10        DirMapas = App.path & "\RECURSOS\MAPAS\"
End Function
            ...
            If ByFlags And 2 Then
                ...
                #If Wgl = 1 Then
                    With GrhData(MapData(X, Y).Graphic(2).GrhIndex)
                        Call g_Swarm.Insert(1, X, Y, .TileWidth, .TileHeight) ' tas usando la nueva sip ahi la cambio
                    End With
                #End If
            Else
            ...
            If ByFlags And 4 Then
                ...
                #If Wgl = 1 Then
                    With GrhData(MapData(X, Y).Graphic(3).GrhIndex)
                        Call g_Swarm.Insert(2, X, Y, .TileWidth, .TileHeight) ' tas usando la nueva sip ahi la cambio
                    End With
                #End If
            Else
            ...
            If ByFlags And 8 Then
                ...
                #If Wgl = 1 Then
                    With GrhData(MapData(X, Y).Graphic(4).GrhIndex)
                        Call g_Swarm.Insert(3, X, Y, .TileWidth, .TileHeight) ' tas usando la nueva sip ahi la cambio
                    End With
                #End If
            Else
...
250      Call modCompression.GenerateContra("3125SA4D4%$$25545!$$/Dassdhg$324dasasddsahtr%43sdfEFSWDret", 0)
...
350              Call _
                     MsgBox("¡Desterium AO ya está corriendo! No es posible correr otra instancia del juego. Haga click en Aceptar para salir.", _
...
#Else
    If Not EngineWgl.Inicialise_Me Then
        Call CloseClient
    End If
...
770         Call Inventario.Initialize(DirectDraw, frmMain.picInv, MAX_INVENTORY_SLOTS, _
          ...
          #Else
            'Inicializamos el inventario gráfico
770         Call Inventario.Initialize(frmMain.picInv, MAX_INVENTORY_SLOTS, _
              , , , , , , , True)
...
810       frmCargando.barra.Width = frmCargando.barra.Width + 85
       ...
       frmConnect.version = "v" & App.Major & "." & App.Minor & " Build: " & _
              App.Revision
      ...
      'TODO : esto de ServerRecibidos no se podría sacar???
       ...
       Call AddtoRichTextBox(frmCargando.status, "Hecho", 255, 0, 0, True, False, _
              False)
...
1170      Call MainTimer.SetInterval(TimersIndex.CastAttack, INT_CAST_ATTACK)
            Call MainTimer.SetInterval(TimersIndex.CastAttack, INT_ATTACK_CAST)
...
Function GetSerialNumber(strDrive As String) As Long '//Disco.
      ...
      Dim Temp1 As String
      Dim Temp2 As String
10    Temp1 = String$(255, Chr$(0))
20    Temp2 = String$(255, Chr$(0))
30    res = GetVolumeInformation(strDrive, Temp1, Len(Temp1), SerialNum, 0, 0, Temp2, _
          Len(Temp2))
40    GetSerialNumber = SerialNum
End Function


Public Sub LoadDataAccount()
On Error GoTo ErrHandler

    With DataAccount
        .Name = GetVar(App.path & "\RECURSOS\DATOS.DS", "ACCOUNT", "NAME")
        .Passwd = GetVar(App.path & "\RECURSOS\DATOS.DS", "ACCOUNT", "PASSWD")
        .State = GetVar(App.path & "\RECURSOS\DATOS.DS", "ACCOUNT", "STATE")
    End With
    Exit Sub
ErrHandler:
End Sub

Public Sub SaveDataAccount(ByVal Name As String, _
...
On Error GoTo ErrHandler
    ...
    With DataAccount
        WriteVar App.path & "\RECURSOS\DATOS.DS", "ACCOUNT", "NAME", Name
        WriteVar App.path & "\RECURSOS\DATOS.DS", "ACCOUNT", "PASSWD", Passwd
        WriteVar App.path & "\RECURSOS\DATOS.DS", "ACCOUNT", "STATE", State
    End With
