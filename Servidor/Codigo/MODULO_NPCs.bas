...
If MiNPC.GivePuntos > 0 Then
UserList(UserIndex).Stats.puntos = UserList(UserIndex).Stats.puntos + MiNPC.GivePuntos
WriteConsoleMsg UserIndex, "¡Por matar al NPC recibes " & MiNPC.GivePuntos & " Canjes!", FontTypeNames.FONTTYPE_ORO
End If
        ...
        If MiNPC.Numero = 697 Then
            FinishCastleMode UserList(UserIndex).flags.SlotEvent, UserList(UserIndex).flags.SlotUserEvent
        End If
        
        ' Npc de invocacion
        If MiNPC.flags.Invocacion = 1 Then
             For i = 1 To NumInvocaciones
                 If Invocaciones(i).NpcIndex = Npclist(NpcIndex).Numero Then
                     Invocaciones(i).Activo = 0
                 End If
             Next i
        End If
...
'***************************************************
    ...
    If Npclist(NpcIndex).Numero = 704 Then
    ...
    If Not toMap Then
        Call WriteCharacterCreate(sndIndex, Npclist(NpcIndex).Char.body, Npclist(NpcIndex).Char.Head, Npclist(NpcIndex).Char.Heading, Npclist(NpcIndex).Char.CharIndex, X, Y, weapon, shield, 0, 0, Helm, vbNullString, 0, 0)
...
'###################################################
    ...
    With Npclist(NpcIndex)
        ...
        .GivePuntos = val(Leer.GetValue("NPC" & NpcNumber, "DaCanjes"))
