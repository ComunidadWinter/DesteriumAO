...
'*************************************************
    ...
    If Modo = eModoComercio.Compra Then
        ...
        ValorCopas = ObjData(Npclist(NpcIndex).Invent.Object(Slot).objindex).copaS * Cantidad
       ValorEldhir = ObjData(Npclist(NpcIndex).Invent.Object(Slot).objindex).Eldhir * Cantidad
       
       ValorCanjes = ObjData(Npclist(NpcIndex).Invent.Object(Slot).objindex).Canjes
        
        If UserList(UserIndex).Stats.puntos < ValorCanjes Then
         Call WriteConsoleMsg(UserIndex, "No tienes canjes suficientes.", FontTypeNames.FONTTYPE_INFO)
         Exit Sub
        End If
        ...
        If TieneObjetos(880, ValorCopas, UserIndex) = False Then
         Call WriteConsoleMsg(UserIndex, "No tienes suficientes Ds Points para negociar conmigo.", FontTypeNames.FONTTYPE_INFO)
         Exit Sub
        End If
        
                If TieneObjetos(943, ValorEldhir, UserIndex) = False Then
         Call WriteConsoleMsg(UserIndex, "No tienes suficiente Eldhires para negociar conmigo.", FontTypeNames.FONTTYPE_INFO)
         Exit Sub
        End If
        ...
        UserList(UserIndex).Stats.puntos = UserList(UserIndex).Stats.puntos - ValorCanjes
        
        If ValorCopas > 0 Then Call QuitarObjetos(880, ValorCopas, UserIndex): Call UpdateUserInv(True, UserIndex, 0)
        If ValorEldhir > 0 Then Call QuitarObjetos(943, ValorEldhir, UserIndex): Call UpdateUserInv(True, UserIndex, 0)
        'If ValorCanjes > 0 Then UserList(UserIndex).Stats.puntos = UserList(UserIndex).Stats.puntos - ValorCanjes
