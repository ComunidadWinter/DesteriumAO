...
10        DAT_APOCALIPSIS = 1
20        DAT_REMUEVE_INVI = 2
...
30        NPCPosX = Npclist(npcind).Pos.X   ''store current position
40        NPCPosY = Npclist(npcind).Pos.Y   ''for direct access
50        NPCPosM = Npclist(npcind).Pos.Map

60        PJBestTarget = False
70        BestTarget = 0
80        quehacer = 0
90        X = 0
100       Y = 0
...
110       If (Npclist(npcind).Stats.MinHp < 750) Then   ''Dying
120           quehacer = 3        ''va a romper su vara en 5 segundos
130       Else
140           If Not (Npclist(npcind).Invent.BarcoSlot = 6) Then
150               Npclist(npcind).Invent.BarcoSlot = 6    ''restore wand break counter
160               Call SendData(SendTarget.ToNPCArea, npcind, PrepareMessageCreateFX(Npclist(npcind).Char.CharIndex, 0, 0))
170           End If
...
180           azar = Sgn(RandomNumber(-1, 1))
              'azar = Sgn(azar)
190           If azar = 0 Then azar = 1
200           azar2 = Sgn(RandomNumber(-1, 1))
              'azar2 = Sgn(azar2)
210           If azar2 = 0 Then azar2 = 1
...
220           For X = NPCPosX + (azar * 8) To NPCPosX + (azar * -8) Step -azar
230               For Y = NPCPosY + (azar2 * 7) To NPCPosY + (azar2 * -7) Step -azar2
240                   NPCAlInd = MapData(NPCPosM, X, Y).NpcIndex  ''por si implementamos algo contra NPCs
250                   PJEnInd = MapData(NPCPosM, X, Y).Userindex

260                   If (PJEnInd > 0) And (Npclist(npcind).CanAttack = 1) Then
270                       If Not (UserList(PJEnInd).flags.Muerto = 1) And Not (UserList(PJEnInd).flags.AdminInvisible = 1) And UserList(PJEnInd).flags.AdminPerseguible Then
280                           If (UserList(PJEnInd).flags.invisible = 1) Or (UserList(PJEnInd).flags.Oculto = 1) Then
                                  ''usuario invisible, vamos a ver si se la podemos sacar

290                               If (RandomNumber(1, 100) <= 35) Then
                                      ''mago detecta invisiblidad
300                                   Npclist(npcind).CanAttack = 0
310                                   Call NPCRemueveInvisibilidad(npcind, PJEnInd, DAT_REMUEVE_INVI)
320                                   Exit Sub    ''basta, SUFICIENTE!, jeje
330                               End If
340                               If UserList(PJEnInd).flags.Paralizado = 1 Then
                                      ''los usuarios invisibles y paralizados son un buen target!
350                                   BestTarget = PJEnInd
360                                   PJBestTarget = True
370                                   quehacer = 2
380                               End If
390                           ElseIf (UserList(PJEnInd).flags.Paralizado = 1) Then
400                               If (BestTarget > 0) Then
410                                   If Not (UserList(BestTarget).flags.invisible = 1 Or UserList(PJEnInd).flags.Oculto = 1) Then
                                          ''encontre un paralizado visible, y no hay un besttarget invisible (paralizado invisible)
420                                       BestTarget = PJEnInd
430                                       PJBestTarget = True
440                                       quehacer = 2
450                                   End If
460                               Else
470                                   BestTarget = PJEnInd
480                                   PJBestTarget = True
490                                   quehacer = 2
500                               End If
510                           ElseIf BestTarget = 0 Then
                                  ''movil visible
520                               BestTarget = PJEnInd
530                               PJBestTarget = True
540                               quehacer = 2
550                           End If  ''
560                       End If  ''endif:    not muerto
570                   End If  ''endif: es un tile con PJ y puede atacar
580               Next Y
590           Next X
600       End If  ''endif esta muriendo


610       Select Case quehacer
              ''case 1 esta "harcodeado" en el doble for
              ''es remover invisibilidades
          Case 2          ''apocalipsis Rahma Nañarak O'al
620           Call SendData(SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead(Hechizos(Npclist(npcind).Spells(DAT_APOCALIPSIS)).PalabrasMagicas, Npclist(npcind).Char.CharIndex, vbCyan))
630           Call NpcLanzaSpellSobreUser2(npcind, BestTarget, Npclist(npcind).Spells(DAT_APOCALIPSIS))    ''SPELL 1 de Mago: Apocalipsis
640       Case 3

650           Call SendData(SendTarget.ToNPCArea, npcind, PrepareMessageCreateFX(Npclist(npcind).Char.CharIndex, FXIDs.FXMEDITARGRANDE, INFINITE_LOOPS))
              ''UserList(UserIndex).Char.FX = FXIDs.FXMEDITARGRANDE

660           If Npclist(npcind).CanAttack = 1 Then
670               Npclist(npcind).CanAttack = 0
680               bs = Npclist(npcind).Invent.BarcoSlot
690               bs = Npclist(npcind).Invent.MonturaSlot
700               bs = bs - 1
710               Call MagoDestruyeWand(npcind, bs, DAT_APOCALIPSIS)
720               If bs = 0 Then
730                   Call MuereNpc(npcind, 0)
740               Else
750                   Npclist(npcind).Invent.BarcoSlot = bs
760                   Npclist(npcind).Invent.MonturaSlot = bs
770               End If
780           End If
790       End Select
...
800       If Npclist(npcind).flags.Paralizado = 1 Then Exit Sub

810       If Not (quehacer = 3) Then      ''si no ta matandose
              ''alejarse si tiene un PJ cerca
              ''pero alejarse sin alejarse del rey
820           If Not (NPCPosM = MAPA_PRETORIANO) Then Exit Sub

              ''Si no hay nadie cerca, o no tengo nada que hacer...
830           If (BestTarget = 0) And (Npclist(npcind).CanAttack = 1) Then Call VolverAlCentro(npcind)

840           PJEnInd = MapData(NPCPosM, NPCPosX - 1, NPCPosY).Userindex
850           If (PJEnInd > 0) Then
860               If Not (UserList(PJEnInd).flags.Muerto = 1) And Not (UserList(PJEnInd).flags.invisible = 1 Or UserList(PJEnInd).flags.Oculto = 1) Then
...
870                   Call VolverAlCentro(npcind)
880                   Exit Sub
890               End If
900           End If

910           PJEnInd = MapData(NPCPosM, NPCPosX + 1, NPCPosY).Userindex
920           If PJEnInd > 0 Then
930               If Not (UserList(PJEnInd).flags.Muerto = 1) And Not (UserList(PJEnInd).flags.invisible = 1 Or UserList(PJEnInd).flags.Oculto = 1) Then
940                   Call VolverAlCentro(npcind)
950                   Exit Sub
960               End If
970           End If

980           PJEnInd = MapData(NPCPosM, NPCPosX, NPCPosY - 1).Userindex
990           If PJEnInd > 0 Then
1000              If Not (UserList(PJEnInd).flags.Muerto = 1) And Not (UserList(PJEnInd).flags.invisible = 1 Or UserList(PJEnInd).flags.Oculto = 1) Then
1010                  Call VolverAlCentro(npcind)
1020                  Exit Sub
1030              End If
1040          End If

1050          PJEnInd = MapData(NPCPosM, NPCPosX, NPCPosY + 1).Userindex
1060          If PJEnInd > 0 Then
1070              If Not (UserList(PJEnInd).flags.Muerto = 1) And Not (UserList(PJEnInd).flags.invisible = 1 Or UserList(PJEnInd).flags.Oculto = 1) Then
1080                  Call VolverAlCentro(npcind)
1090                  Exit Sub
1100              End If
1110          End If


1120      End If  ''end if not matandose

1130      Exit Sub

PRMAGO_AI_Error:

    LogError "Error " & Err.Number & " (" & Err.description & ") in procedure PRMAGO_AI, line " & Erl & "."
