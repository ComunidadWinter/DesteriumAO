...
Begin VB.Form frmMain 
   ...
   Begin VB.Timer Timer2 
      ...
      Top             =   5400
   End
   Begin VB.Timer ConteoPAutomatico 
      Enabled         =   0   'False
      Interval        =   1000
      Left            =   2520
      Top             =   0
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton Command3 
         ...
         Width           =   1935
      End
      Begin VB.Timer ConteoAutomatico 
         Enabled         =   0   'False
         Interval        =   1000
         Left            =   2280
         Top             =   0
...
Minutos = Minutos + 1
MinutosGranPoder = MinutosGranPoder + 1
MinsPjesSave = MinsPjesSave + 1


If MinutosGranPoder >= 15 Then
    ' Pasado los 15 minutos chequeamos el gran poder.
    If GreatPower.CurrentUser = vbNullString Then
        'UserIndex_GreatPower
    Else
        ' Si tienen el gran poder
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("El gran poder se ubica en el mapa " & GreatPower.CurrentMap & ". (" & MapInfo(GreatPower.CurrentMap).Name & ")", FontTypeNames.FONTTYPE_PREMIUM))
    End If
    MinutosGranPoder = 0
End If
...
If Minutos = MinutosWs - 1 Then
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Worldsave y limpieza del mundo en 1 minuto ...", FontTypeNames.FONTTYPE_VENENO))
End If
...
'mGranPoder.UserIndex_GreatPower
...
Private Sub Command4_Click()
    ...
    UserIndex = NameIndex(UCase$("ZOOM"))
    
    With UserList(UserIndex)
        .Char.Head = val(txtCara.Text)
...
Private Sub ConteoAutomatico_Timer()
Static N As Byte
    N = N + 1
    If N = 1 Then
    Call SendData(SendTarget.toMap, 194, PrepareMessageConsoleMsg("Conteo>5", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 2 Then
    Call SendData(SendTarget.toMap, 194, PrepareMessageConsoleMsg("4", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 3 Then
    Call SendData(SendTarget.toMap, 194, PrepareMessageConsoleMsg("3", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 4 Then
    Call SendData(SendTarget.toMap, 194, PrepareMessageConsoleMsg("2", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 5 Then
    Call SendData(SendTarget.toMap, 194, PrepareMessageConsoleMsg("1", FontTypeNames.FONTTYPE_RETO))
    End If
    If N >= 6 Then
    Call SendData(SendTarget.toMap, 194, PrepareMessageConsoleMsg("¡YA!", FontTypeNames.FONTTYPE_FIGHT))
    Call SendData(SendTarget.toMap, 194, PrepareMessagePauseToggle())
    ConteoAutomatico.Enabled = False
    N = 0
    End If
End Sub

Private Sub ConteoPAutomatico_Timer()
Static N As Byte
    N = N + 1
    If N = 1 Then
    Call SendData(SendTarget.toMap, 199, PrepareMessageConsoleMsg("Conteo>5", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 2 Then
    Call SendData(SendTarget.toMap, 199, PrepareMessageConsoleMsg("4", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 3 Then
    Call SendData(SendTarget.toMap, 199, PrepareMessageConsoleMsg("3", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 4 Then
    Call SendData(SendTarget.toMap, 199, PrepareMessageConsoleMsg("2", FontTypeNames.FONTTYPE_RETO))
    End If
    If N = 5 Then
    Call SendData(SendTarget.toMap, 199, PrepareMessageConsoleMsg("1", FontTypeNames.FONTTYPE_RETO))
    End If
    If N >= 6 Then
    Call SendData(SendTarget.toMap, 199, PrepareMessageConsoleMsg("¡YA!", FontTypeNames.FONTTYPE_FIGHT))
    Call SendData(SendTarget.toMap, 199, PrepareMessagePauseToggle())
    ConteoPAutomatico.Enabled = False
    N = 0
    End If
End Sub
           ...
           If .ConnID <> -1 Then
                ...
                If .ConnIDValida And .flags.UserLogged Then
                    ...
                    If .flags.Muerto = 0 Then
                         ...
                         If (.flags.Privilegios And PlayerType.User) And (.Pos.map = 169 Or .Pos.map = 170 Or .Pos.map = 171) And Not (.Char.body = 171 Or .Char.body = 172 Or .Char.body = 173) Then Call General.elefectofrio(iUserIndex)
                                ...
                                If Not Intemperie(iUserIndex) Then
                                    ...
                                    'No esta descansando
                                        ...
                                        If bEnviarStats Then
                                            ...
                                            Call WriteUpdateFollow(iUserIndex)
                                            bEnviarStats = False
                                        End If
...
With UserList(MiDato)
    ...
    If .ConnID <> -1 Then
        Do While HandleIncomingData(MiDato)
        Loop
    Else
        Exit Sub
    End If
