...
With UserList(UserIndex)
    ...
    If EsAdmin(Name) Then
        ...
        Call LogGM(Name, "Se conecto con ip:" & .ip & "/" & .CPU_ID & "/" & .HD)
    ...
    Call LogUserConect(Name, "Se conecto con ip:" & .ip & "/" & .CPU_ID & "/" & .HD)
...
880     .StaticHD = Leer.GetValue("INIT", "StaticHD")

890     If LenB(.StaticHD) <> 0 Then
900         If Not .StaticHD = .HD Then
910             Call WriteErrorMsg(UserIndex, "Este personaje está protegido.")
930             Call FlushBuffer(UserIndex)
940             Call CloseSocket(UserIndex)
...
970     End If
    ...
    If .Invent.EscudoEqpSlot > 25 Then .Invent.EscudoEqpSlot = NingunEscudo
    If .Invent.CascoEqpSlot > 25 Then .Invent.CascoEqpSlot = NingunCasco
    If .Invent.WeaponEqpSlot > 25 Then .Invent.WeaponEqpSlot = NingunArma
   ...
   'If in the water, and has a boat, equip it!
    ...
    Call WriteConsoleMsg(UserIndex, ">Bienvenido a Desterium AO noble aventurero.", FontTypeNames.FONTTYPE_GUILD)
    Call WriteConsoleMsg(UserIndex, ">El máximo de oro es 50.000.000 por personaje.", FontTypeNames.FONTTYPE_GM)
    Call WriteConsoleMsg(UserIndex, ">Recuerda visitar nuestro sitio oficial: http://www.Desterium-ao.jimdosite.com/", FontTypeNames.FONTTYPE_GUILD)
    ...
    Call WriteConsoleMsg(UserIndex, ">El uso de cheats será sancionado de forma estricta por el Staff.", FontTypeNames.FONTTYPE_EJECUCION)
    ...
    If .Mercado.InList > 0 And _
        (.Mercado.Gld > 0 Or .Mercado.Dsp > 0) Then
        WriteConsoleMsg UserIndex, "¡¡ATENCIÓN!! Tu personaje esta en modo VENTA por DSP o MONEDAS DE ORO. No podrán comprartelo. /QUITARPJ para quitarlo del MERCADO.", FontTypeNames.fonttype_dios
    End If
     
    
    Call CheckLogros(UserIndex)
...
'*************************************************
    ...
    With UserList(UserIndex).PosAnt
        .map = 0
        .X = 0
        .Y = 0
    End With
...
Call ResetUserLogros(UserIndex)
