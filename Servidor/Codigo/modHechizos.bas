...
Sub LanzarHechizo(ByVal SpellIndex As Integer, ByVal Userindex As Integer)
    On Error GoTo LanzarHechizo_Error
      '***************************************************
      'Autor: Unknown (orginal version)
...
20     If Hechizos(SpellIndex).Nombre = "Implorar Ayuda" And .clase <> eClass.Druid Then
30          WriteConsoleMsg Userindex, "No eres un druida para invocar este hechizos!", FontTypeNames.FONTTYPE_INFO
40          Exit Sub
50      End If
          
          
60          'If .death = True And DeathMatch.Cuenta > 0 Then
70              '  WriteConsoleMsg UserIndex, "¡No puedes atacar antes de la cuenta regresiva!", FontTypeNames.FONTTYPE_INFO
80              '  Exit Sub
90            'End If
              
100      ' If (DeathMatch.Ingresaron < DeathMatch.Cupos And .death = True) Then
110       'WriteConsoleMsg UserIndex, "No puedes atacar si no se llenaron los cupos", FontTypeNames.FONTTYPE_WARNING
120       'Exit Sub
130       'End If
          
140                       If .hungry = True And JDH.Cuenta > 0 Then
150               WriteConsoleMsg Userindex, "¡No puedes atacar antes de la cuenta regresiva!", FontTypeNames.FONTTYPE_INFO
160               Exit Sub
170           End If
              
180       If (JDH.Ingresaron < JDH.Cupos And .hungry = True) Then
190       WriteConsoleMsg Userindex, "No puedes atacar si no se llenaron los cupos", FontTypeNames.FONTTYPE_WARNING
200       Exit Sub
210       End If
          
220       If .flags.EnConsulta Then
230           Call WriteConsoleMsg(Userindex, "No puedes lanzar hechizos si estás en consulta.", FontTypeNames.FONTTYPE_INFO)
240           Exit Sub
250       End If
          
260       If PuedeLanzar(Userindex, SpellIndex) Then
270           Select Case Hechizos(SpellIndex).Target
                  Case TargetType.uUsuarios
280                   If .flags.TargetUser > 0 Then
290                       If Abs(UserList(.flags.TargetUser).Pos.Y - .Pos.Y) <= RANGO_VISION_Y Then
300                           Call HandleHechizoUsuario(Userindex, SpellIndex)
310                       Else
320                           Call WriteConsoleMsg(Userindex, "Estás demasiado lejos para lanzar este hechizo.", FontTypeNames.FONTTYPE_WARNING)
330                       End If
340                   Else
350                       Call WriteConsoleMsg(Userindex, "Este hechizo actúa sólo sobre usuarios.", FontTypeNames.FONTTYPE_INFO)
360                   End If
                  
370               Case TargetType.uNPC
380                   If .flags.TargetNPC > 0 Then
390                       If Abs(Npclist(.flags.TargetNPC).Pos.Y - .Pos.Y) <= RANGO_VISION_Y Then
400                           Call HandleHechizoNPC(Userindex, SpellIndex)
410                       Else
420                           Call WriteConsoleMsg(Userindex, "Estás demasiado lejos para lanzar este hechizo.", FontTypeNames.FONTTYPE_WARNING)
430                       End If
440                   Else
450                       Call WriteConsoleMsg(Userindex, "Este hechizo sólo afecta a los npcs.", FontTypeNames.FONTTYPE_INFO)
460                   End If
                  
470               Case TargetType.uUsuariosYnpc
480                   If .flags.TargetUser > 0 Then
490                       If Abs(UserList(.flags.TargetUser).Pos.Y - .Pos.Y) <= RANGO_VISION_Y Then
500                           Call HandleHechizoUsuario(Userindex, SpellIndex)
510                       Else
520                           Call WriteConsoleMsg(Userindex, "Estás demasiado lejos para lanzar este hechizo.", FontTypeNames.FONTTYPE_WARNING)
530                       End If
540                   ElseIf .flags.TargetNPC > 0 Then
550                       If Abs(Npclist(.flags.TargetNPC).Pos.Y - .Pos.Y) <= RANGO_VISION_Y Then
560                           Call HandleHechizoNPC(Userindex, SpellIndex)
570                       Else
580                           Call WriteConsoleMsg(Userindex, "Estás demasiado lejos para lanzar este hechizo.", FontTypeNames.FONTTYPE_WARNING)
590                       End If
600                   Else
610                       Call WriteConsoleMsg(Userindex, "Target inválido.", FontTypeNames.FONTTYPE_INFO)
620                   End If
                  
630               Case TargetType.uTerreno
640                   Call HandleHechizoTerreno(Userindex, SpellIndex)
650           End Select
              
660       End If
          
670       If .Counters.Trabajando Then _
              .Counters.Trabajando = .Counters.Trabajando - 1
          
680       If .Counters.Ocultando Then _
...
With UserList(Userindex)
    ...
    If Hechizos(HechizoIndex).Paraliza = 1 Or Hechizos(HechizoIndex).Inmoviliza = 1 Then
        ...
        If UserList(TargetIndex).flags.SlotEvent > 0 Then
            If Events(UserList(TargetIndex).flags.SlotEvent).Users(UserList(TargetIndex).flags.SlotUserEvent).Selected = 1 Then
                Call WriteConsoleMsg(Userindex, "El personaje no puede ser paralizado en este evento.", FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
        End If
    ...
    If Hechizos(HechizoIndex).Revivir = 1 Then
        ...
        'revisamos si necesita vara
            ...
            ElseIf .clase = eClass.Bard Then
                If .Invent.MunicionEqpObjIndex <> LAUDELFICO And _
   ...
   End If
    ...
    ElseIf Hechizos(SpellIndex).SubeHP = 2 Then
        ...
        End If
            ...
            daño = daño * 1.1   'laud magico de los bardos
        End If
    ...
    With UserList(Userindex)
        ...
        If tUser > 0 Then
            If Userindex <> tUser Then
                If .showName And Not UserList(tUser).flags.SlotEvent > 0 Then
...
With UserList(TargetIndex)
    ...
    ElseIf Hechizos(SpellIndex).SubeHP = 2 Then
        ...
        End If
         ...
         If UserList(Userindex).Invent.MunicionEqpObjIndex = LAUDSUPERMAGICO Or UserList(Userindex).Invent.MunicionEqpObjIndex = FLAUTAANTIGUA Then
            daño = daño * 1.08  'laud magico de los bardos
        End If
        ...
        ' PRIMER CASO: LANZADOR TIENE EL PODER
        If StrComp(GreatPower.CurrentUser, UCase$(UserList(Userindex).Name)) = 0 Then
            daño = daño * 1.1
        End If
