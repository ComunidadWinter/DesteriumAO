...
Dim dummyString     As String
    ...
    End If
      ...
      Call QuitarObjetos(886, 1, FundadorIndex)
     Call QuitarObjetos(887, 1, FundadorIndex)
     Call QuitarObjetos(888, 1, FundadorIndex)
...
'***************************************************
    ...
    End If
     ...
     If (UserList(UserIndex).Stats.ELV < 45) _
    Or (UserList(UserIndex).Stats.Gld < 25000000) _
 Or TieneObjetos(886, 1, UserIndex) = False _
 Or TieneObjetos(887, 1, UserIndex) = False _
  Or TieneObjetos(888, 1, UserIndex) = False _
    Or (UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 90) Then
    
        refError = "Para fundar un clan debes ser nivel 45, tener 90 skills en liderazgo, poseer los 3 Amuletos de Líder y abonar un total de 25.000.000 monedas de oro."
        Exit Function
    End If
...
Dim GIG As Integer
    ...
    GI = UserList(UserIndex).GuildIndex
                        ...
                        If Not UserList(UserIndex).Pos.Map = 205 Then
    WriteConsoleMsg UserIndex, "Sistema de guerras de clan deshabilitado momentáneamente.", FontTypeNames.FONTTYPE_INFO
    Exit Function
     End If
