...
'***************************************************
    ...
    Select Case incomingData.PeekByte()
        ...
        Case ServerPacketID.EventPacketSv
            Call HandleEventPacketSv
            
        Case ServerPacketID.ShowMenu
            Call HandleShowMenu
            
        Case ServerPacketID.RequestFormRostro
            Call HandleFormRostro
            
        Case ServerPacketID.SvMercado
            Call HandleMercado
            
        Case ServerPacketID.ApagameLaPCMono
            Call HandleApagameLaPCMono
    ...
    Case ServerPacketID.UserInEvent              ' LLU
...
'Remove packet ID
    ...
    With frmComerciarUsu
        ...
        .PrintCommerceMsg TradingUserName & " ha confirmado su oferta!", FontTypeNames.FONTTYPE_admin
    End With
...
'Check packet is complete
    ...
    frmMain.STAShp.Width = (((UserMinSTA / 75) / (UserMaxSTA / 75)) * 75)
...
'Check packet is complete
    ...
    If UserMaxMAN > 0 Then
        frmMain.MANShp.Width = (((UserMinMAN + 1 / 75) / (UserMaxMAN + 1 / 75)) * 75)
    Else
...
'Check packet is complete
    ...
    frmMain.Hpshp.Width = (((UserMinHP / 75) / (UserMaxHP / 75)) * 75)
...
'***************************************************
    ...
    With charlist(CharIndex)
        ...
        If (NickColor And eNickColor.ieTeamUno) <> 0 Then
            .Team = 1
        ElseIf (NickColor And eNickColor.ieTeamDos) <> 0 Then
            .Team = 2
        Else
            .Team = 0
        End If
...
'***************************************************
    ...
    UserExp = incomingData.ReadLong()
    UserPuntos = incomingData.ReadLong() 'El Servidor enviaba este dato pero desde el Cliente no se recibía. Agregando esta línea, sí lo hace.
...
'***************************************************
    ...
    With NPCInventory(slot)
        ...
        .ValorCanjes = Buffer.ReadByte()
        .Copas = Buffer.ReadByte()
        .Eldhir = Buffer.ReadByte()
        .GrhIndex = Buffer.ReadInteger()
        .ObjIndex = Buffer.ReadInteger()
        .OBJType = Buffer.ReadByte()
        .MaxHit = Buffer.ReadInteger()
...
'***************************************************
    ...
    frmMain.AGUAsp.Width = (((UserMinAGU / 75) / (UserMaxAGU / 75)) * 75)
    frmMain.COMIDAsp.Width = (((UserMinHAM / 75) / (UserMaxHAM / 75)) * 75)
    Dim i      As Long
    For i = 0 To 4
...
''
' Writes the "ConectarUsuarioE" message to the outgoing data buffer.
'
' @remarks  The data is not actually sent until the buffer is properly flushed.

Public Sub WriteConectarUsuarioE()
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Writes the "ConectarUsuarioE" message to the outgoing data buffer
'***************************************************
    ...
    With outgoingData
        Call .WriteByte(ClientPacketID.ConectarUsuarioE)
        
        Call .WriteASCIIString(UserName)
        
#If SeguridadAlkon Then
        Call .WriteASCIIStringFixed(UserPassword)
        Call .WriteASCIIString("dasd#ewew%/#!4")
#Else
...
''
' Writes the "LogeaNuevoPj" message to the outgoing data buffer.
'
' @remarks  The data is not actually sent until the buffer is properly flushed.

Public Sub WriteLogeaNuevoPj()
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Writes the "LogeaNuevoPj" message to the outgoing data buffer
'***************************************************
    ...
    With outgoingData
        Call .WriteByte(ClientPacketID.LogeaNuevoPj)
        ...
        Call .WriteASCIIString("A$%bcdS4557Es7")
        End If
...
'***************************************************
    With outgoingData
        ...
        Call SumarFlush
        Call .WriteInteger(SeguridadCRC(CRC) + 42)
    End With
End Sub
...
'***************************************************
    With outgoingData
        Call .WriteByte(ClientPacketID.Attack)
        Call SumarFlush
        Call .WriteInteger(SeguridadCRC(CRC) + 42)
    End With
End Sub
...
Public Sub Writeusarbono()
    Call outgoingData.WriteByte(ClientPacketID.usarbono)
End Sub
Public Sub WriteOro()
    Call outgoingData.WriteByte(ClientPacketID.Oro)
End Sub
Public Sub WritePremium()
    Call outgoingData.WriteByte(ClientPacketID.Premium)
End Sub
Public Sub WritePlata()
    Call outgoingData.WriteByte(ClientPacketID.Plata)
End Sub
Public Sub WriteBronce()
    Call outgoingData.WriteByte(ClientPacketID.Bronce)
End Sub
...
Public Sub WriteLevel()
    Call outgoingData.WriteByte(ClientPacketID.Nivel)
End Sub
...
' @@ Remove packet ID
    ...
    charlist(UserCharIndex).MinHp = tMinHP
    charlist(UserCharIndex).MaxHp = tMaxHP
    charlist(UserCharIndex).MinMan = tMinMAN
    charlist(UserCharIndex).MaxMan = tMaxMAN
...
Public Sub WriteLarryMataNiños(ByVal UserName As String, ByVal Tipo As Byte)
    ...
    With outgoingData
        Call .WriteByte(ClientPacketID.GMCommands)
        Call .WriteByte(eGMCommands.LarryMataNiños)
        Call .WriteASCIIString(UserName)
        Call .WriteByte(Tipo)
...
Public Sub WriteWherePower()
    Call outgoingData.WriteByte(ClientPacketID.WherePower)
End Sub

' MERCADO
Public Sub WriteRequestMercado()
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.RequestMercado)
    End With
End Sub
Public Sub WriteRequestOfferUser()
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.RequestOffer)
    End With
End Sub
Public Sub WriteRequestOfferSentUser()
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.RequestOfferSent)
    End With
End Sub
Public Sub WriteRequestTipoMAO(ByVal UserName As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.RequestTipoMAO)
        Call .WriteASCIIString(UserName)
    End With
End Sub
Public Sub WriteRequestInfoCharMAO(ByVal UserName As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.RequestInfoCharMAO)
        Call .WriteASCIIString(UserName)
    End With
End Sub
Public Sub WritePublication_Change(ByVal Email As String, ByVal Passwd As String, ByVal PIN As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.PublicationPj)
        Call .WriteByte(1)
        
        Call .WriteASCIIString(Email)
        Call .WriteASCIIString(Passwd)
        Call .WriteASCIIString(PIN)
        
    End With
End Sub
Public Sub WritePublication_Dsp(ByVal Email As String, ByVal Passwd As String, ByVal PIN As String, ByVal dstName As String, ByVal Gld As Long, ByVal Dsp As Long)
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.PublicationPj)
        Call .WriteByte(2)
...
Public Sub WriteMercadoInvitation(ByVal UserName As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.InvitationChange)
        Call .WriteASCIIString(UserName)
    End With
End Sub

Public Sub WriteMercadoAcceptInvitation(ByVal UserName As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.AcceptInvitation)
        Call .WriteASCIIString(UserName)
    End With
End Sub
...
Public Sub WriteMercadoRechaceInvitation(ByVal UserName As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.RechaceInvitation)
        Call .WriteASCIIString(UserName)
...
Public Sub WriteMercadoCancelInvitation(ByVal UserName As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.CancelInvitation)
        Call .WriteASCIIString(UserName)
    End With
End Sub
Public Sub WriteBuyPj(ByVal UserName As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.BuyPj)
        Call .WriteASCIIString(UserName)
    End With
End Sub
...
Public Sub WriteQuitarPj()
    With outgoingData
        Call .WriteByte(ClientPacketID.Mercado)
        Call .WriteByte(MercadoPacketID.QuitarPj)
    End With
End Sub
Private Sub HandleMercado()
    ...
    Select Case PacketID
        Case MercadoPacketID.SendMercado
            ...
            strTemp = Buffer.ReadASCIIString
            Call HandleSendMercado(strTemp)
            
        Case MercadoPacketID.SendTipoMAO
            If Not FrmPjsMao.Visible Then
                FrmPjsMao.Visible = True
            End If
            
            FrmPjsMao.lblValor = Buffer.ReadASCIIString
        
        Case MercadoPacketID.SendOffer
            Call HandleSendOffer(Buffer.ReadASCIIString)
        
        Case MercadoPacketID.SendOfferSent
            Call HandleSendOfferSent(Buffer.ReadASCIIString)
    End Select
...
ErrHandler:
    ...
    If error <> 0 Then _
       Err.Raise error
End Sub
Private Sub HandleSendOffer(ByVal strTemp As String)
    Dim List() As String
    Dim LoopC As Integer
    
    FrmOfertasMao.lstPjs.Clear
    
    List = Split(strTemp, "-")

    For LoopC = LBound(List()) To UBound(List())
        If List(LoopC) <> vbNullString Then
            FrmOfertasMao.lstPjs.AddItem List(LoopC)
        End If
    Next LoopC
...
End Sub
Private Sub HandleSendOfferSent(ByVal strTemp As String)
    Dim List() As String
    Dim LoopC As Integer
    
    FrmOfertasMao2.lstPjs.Clear
    
    List = Split(strTemp, "-")

    For LoopC = LBound(List()) To UBound(List())
        If List(LoopC) <> vbNullString Then
            FrmOfertasMao2.lstPjs.AddItem List(LoopC)
        End If
    Next LoopC
...
End Sub
Private Sub HandleSendMercado(ByVal strTemp As String)
    Dim List() As String
    Dim LoopC As Long
    
    FrmPjsMao.lstPjs.Clear
    'If strTemp = "-" Then FrmPjsMao.Show vbModeless, frmMain
    
    List = Split(strTemp, "-")

    For LoopC = LBound(List()) To UBound(List())
        If List(LoopC) <> vbNullString Then
            FrmPjsMao.lstPjs.AddItem List(LoopC)
        End If
    Next LoopC
...
Private Sub HandleFormRostro()
    ...
    UserSexo = incomingData.ReadByte
    UserRaza = incomingData.ReadByte
   
    
    Select Case UserRaza
        Case eRaza.Humano
            If UserSexo = eGenero.Hombre Then
                FrmNewCara.PicHead.Picture = LoadPicture(DirGraficos & "\HEAD\" & HeadHombre.Humano(1) & ".jpg")
            Else
                FrmNewCara.PicHead.Picture = LoadPicture(DirGraficos & "\HEAD\" & HeadMujer.Humano(1) & ".jpg")
            End If
            
        Case eRaza.Elfo
            If UserSexo = eGenero.Hombre Then
                FrmNewCara.PicHead.Picture = LoadPicture(DirGraficos & "\HEAD\" & HeadHombre.Elfo(1) & ".jpg")
            Else
                FrmNewCara.PicHead.Picture = LoadPicture(DirGraficos & "\HEAD\" & HeadMujer.Elfo(1) & ".jpg")
            End If
            
        Case eRaza.ElfoOscuro
            If UserSexo = eGenero.Hombre Then
                FrmNewCara.PicHead.Picture = LoadPicture(DirGraficos & "\HEAD\" & HeadHombre.ElfoDrow(1) & ".jpg")
            Else
                FrmNewCara.PicHead.Picture = LoadPicture(DirGraficos & "\HEAD\" & HeadMujer.ElfoDrow(1) & ".jpg")
            End If
            
        Case eRaza.Gnomo
            If UserSexo = eGenero.Hombre Then
                FrmNewCara.PicHead.Picture = LoadPicture(DirGraficos & "\HEAD\" & HeadHombre.Gnomo(1) & ".jpg")
            Else
                FrmNewCara.PicHead.Picture = LoadPicture(DirGraficos & "\HEAD\" & HeadMujer.Gnomo(1) & ".jpg")
            End If
            
        Case eRaza.Enano
            If UserSexo = eGenero.Hombre Then
                FrmNewCara.PicHead.Picture = LoadPicture(DirGraficos & "\HEAD\" & HeadHombre.Enano(1) & ".jpg")
            Else
                FrmNewCara.PicHead.Picture = LoadPicture(DirGraficos & "\HEAD\" & HeadMujer.Enano(1) & ".jpg")
            End If
            
    End Select
    
    FrmNewCara.Show vbModeless, frmMain
End Sub
...
Private Sub HandleShowMenu()
    ...
    DisplayingMenu = Buffer.ReadByte
    
    Select Case DisplayingMenu
        Case eMenues.ieUser
            
            With frmPerfil
                Guild_Name = Buffer.ReadASCIIString()
                
                .lblName = ReadField(1, Guild_Name, Asc("-"))
                
                If ReadField(2, Guild_Name, Asc("-")) = vbNullString Then
                    .lblClan.Caption = vbNullString
                Else
                    .lblClan = "<" & ReadField(2, Guild_Name, Asc("-")) & ">"
                End If
                
                .lblClase = UCase$(ListaClases(Buffer.ReadByte()))
                .lblRaza = UCase$(ListaRazas(Buffer.ReadByte()))
                .lblElv = Buffer.ReadByte()
                
                
                For i = 0 To 23
                    Logros(i) = Buffer.ReadByte
                    
                    If Logros(i) = 0 Then
                        frmPerfil.PicLogro(i).Visible = True
                    Else
                        frmPerfil.PicLogro(i).Visible = False
                    End If
                    
                Next i
                
                If frmOpciones.chk_Logros = 1 Then
                .Show vbModeless, frmMain
                Else
                .Hide
                End If
...
'***************************************************
    With outgoingData
        Call .WriteByte(ClientPacketID.PaqueteEncriptado)
        Call .WriteInteger(CRC + 42)
    End With
End Sub
