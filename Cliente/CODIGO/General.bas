...
Private Type tConfig
    NotWalkToConsole As Byte 'benjabarrera
    NotLogros As Byte 'benjabarrera
End Type
...
'***************************************************
    ...
    If Config.NotWalkToConsole Then
        If frmMain.SendTxt.Visible = True Or frmMain.SendCMSTXT.Visible = True Then
            If Not frmMain.SendTxt.Text = vbNullString And Not frmMain.SendTxt.Text = " " And Not frmMain.SendTxt.Text = "  " Then
                Exit Sub
            End If
        End If
    End If
...
Sub SwitchMap(ByVal map As Integer)
    ...
    If FileExist(DirMapas & "tmp.map", vbArchive) Then Kill DirMapas & "tmp.map"
...
If Cpu_SSN = vbNullString Then Cpu_SSN = SystemSerialNumber64
    ...
    Call modCompression.GenerateContra("244433783", 0) ' graficos
   Call modCompression.GenerateContra("DMapas1", 1) ' mapas
    ...
    'If FindPreviousInstance Then
      ' Call MsgBox("Desterium  AO ya está corriendo! No es posible correr otra instancia del juego. Haga click en Aceptar para salir.", vbApplicationModal + vbInformation + vbOKOnly, "Error al ejecutar")
       'End
   'End If
...
#End If
    ...
    Call AddtoRichTextBox(frmCargando.status, "                    ¡Bienvenido a Desterium  AO!", 255, 255, 255, True, False, True)
    frmCargando.barra.Width = frmCargando.barra.Width + 85
...
Public Sub LoadConfig()
    With Config
        .NotWalkToConsole = GetVar(App.path & "\INIT\ConfigD.DAT", "CONFIG", "NOTWALK")
        .NotLogros = GetVar(App.path & "\INIT\ConfigD.DAT", "CONFIG", "NOTLOGROS")
    End With
End Sub
