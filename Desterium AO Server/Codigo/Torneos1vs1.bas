...
On Error GoTo errorh:
    ...
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("1vs1> Se canceló por falta de participantes.", FontTypeNames.FONTTYPE_GUILD))
    Dim i As Integer
...
End If
    ...
    If (Torneo_Rondas = 1) Then
        ...
        'Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("1vs1> " & UserList(UserIndex).Name & " pierde el combate!", FontTypeNames.FONTTYPE_GUILD))
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("1vs1> El ganador del torneo es " & UserList(Torneo_Luchadores(LI1)).Name & " que se lleva un total de 1 punto de torneo y 150.000 monedas de oro. ¡FELICIDADES!", FontTypeNames.FONTTYPE_CONSEJOVesA))
        ...
        UserList(Torneo_Luchadores(LI1)).Stats.GLD = UserList(Torneo_Luchadores(LI1)).Stats.GLD + 150000
    ...
    End If
        ...
        If (2 ^ Torneo_Rondas = 2 * combate) Then
                ...
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("1vs1> Siguiente ronda!", FontTypeNames.FONTTYPE_GUILD))
...
If (Torneo_Activo) Then
        ...
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("1vs1> Torneo iniciado con un cupo de " & val(2 ^ rondas) & " participantes. Para participar teclea /INGRESAR - (No cae inventario).", FontTypeNames.FONTTYPE_GUILD))
...
On Error GoTo errorh
        ...
        If (Torneo_Activo) Then
                ...
                Exit Sub
        End If
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("1vs1> Esta empezando un nuevo torneo 1vs1 de " & val(2 ^ rondas) & " participantes!! para participar teclea /INGRESAR - (No cae inventario)", FontTypeNames.FONTTYPE_GUILD))
...
Dim UI1 As Integer, UI2 As Integer
    ...
    If (UI1 = -1) Then
        ...
        End If
        If (UI2 <> -1) Then _
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("1vs1> El usuario " & UserList(UI2).Name & " pasó a la siguiente ronda.", FontTypeNames.FONTTYPE_GUILD))
        ...
        Exit Sub
    End If
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("1vs1> " & UserList(UI1).Name & " vs. " & UserList(UI2).Name & ".¡Esquinas!... ¡A luchar!", FontTypeNames.FONTTYPE_GUILD))
